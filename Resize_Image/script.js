const unloadBox = document.querySelector(".upload-box");
previewImg = unloadBox.querySelector("img");
fileInput = unloadBox.querySelector("input");
...
const loadFile = (e) => {
    const file = e.target.files[0]; // lấy file img
    if (!file) return; // !file thì dừng lại
    previewImg.src = URL.createObjectURL(file); // gán cái file URL qua img src
    previewImg.addEventListener("load", () => {
        widthInput.value = previewImg.naturalWidth; // tra ve chieu rong cua buc anh
...
unloadBox.addEventListener("click", () => fileInput.click());
